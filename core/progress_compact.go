...
func progressStyleFor(p Platform, display DisplayCfg) string {
...
func SuppressStandaloneToolResultEvent(p Platform, display DisplayCfg) bool {
	...
	return progressStyleFor(p, display) == progressStyleLegacy
}

func newCompactProgressWriter(ctx context.Context, p Platform, replyCtx any, agentName string, lang Language, display DisplayCfg) *compactProgressWriter {
	w := &compactProgressWriter{
		ctx:        ctx,
		platform:   p,
		replyCtx:   replyCtx,
		style:      progressStyleFor(p, display),
		state:      ProgressCardStateRunning,
